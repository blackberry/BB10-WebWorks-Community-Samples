    ...
    , _expireTimer(new QTimer(this))
{
    ...
    if(!QObject::connect(_socketListener, SIGNAL(inboundMessage(const QString &)),
                                    this, SLOT(onInboundMessage(const QString &)))) {
        qWarning() << "SSSS Service::Service() - connect failed - onInboundMessage" << strerror(errno) << endl;
    }
...
void Service::onInvoked(const bb::system::InvokeRequest &request)
{
    ...
    if (request.action().compare(WAKEME_INVOKE_ACTION_RESET) == 0) {
        ...
        bt_le_add_scan_device(getInvokeBluetoothAddress(request.data()), this);

    } else if (request.action().compare(WAKEME_INVOKE_ACTION_REMOVESCAN) == 0) {

        bt_le_remove_scan_device(getInvokeBluetoothAddress(request.data()));
...
QByteArray Service::getInvokeBluetoothAddress(const QByteArray &invokeData)
{
    qDebug() << "SSSS Received data=" << invokeData.data();

    QString address = DEFAULT_BLUETOOTH_ADDRESS;

    JsonDataAccess jda;
    QVariant parsedPayload = jda.loadFromBuffer(invokeData);
    if (!jda.hasError()) {
        QVariantMap payload = parsedPayload.toMap();
        address = payload["address"].toString();
        qDebug() << "SSSS Using requested address=" << address << endl;
    } else {
        qDebug() << "SSSS Requested Address parse error: " << jda.error().errorMessage() << endl;
        qDebug() << "SSSS Using default address=" << address << endl;
    }
...
void Service::onInboundMessage(const QString &message)
{
    JsonDataAccess jda;
    QVariant parsedPayload = jda.loadFromBuffer(message);
    if (!jda.hasError()) {
        QVariantMap payload = parsedPayload.toMap();
        QString type = payload["TYPE"].toString();
        if (type.compare("PING") == 0) {
            qDebug() << "SSSS Message from GUI:" << payload["DATA"].toString() << endl;
            sendStatus();
        }
    } else {
        qWarning() << "SSSS Service::onInboundMessage() - JSON parse error" << message << endl;
    }
}
